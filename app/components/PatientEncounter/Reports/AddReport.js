...
function AddReport(props) {
  ...
  const { open, encounterData, closeForm, data, type, callBack, setMessage } =
    props;
  ...
  const postReports = async (values, setErrors, setStatus, setSubmitting) => {
    ...
    >
      <Formik
        initialValues={{
          ...
          reportRef:
            Object.keys(editData).length > 0 ? editData["report_id"] : "",
          investigation:
            Object.keys(editData).length > 0 ? editData["investigation"] : "",
          investigationDate:
            Object.keys(editData).length > 0
              ? moment(editData["investigation_date"]).format("YYYY-MM-DD")
              : moment(new Date()).format("YYYY-MM-DD"),
          investigationType:
            Object.keys(editData).length > 0
              ? editData["investigation_type"]
              : "",
