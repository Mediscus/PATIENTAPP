...
function AreaComponent(_ref) {
  ...
  return React__default.createElement("g", {
    ...
    var areaPath = (_area$curve = d3Shape.area(function (datum) {
      var _getX;
    ...
    var linePath = (_line$curve = d3Shape.line(function (datum) {
      var _getX2;
...
function buildTimeAxis(options, series, isVertical, range, outerRange) {
  var scaleFn = options.scaleType === 'localTime' ? d3Scale.scaleTime : d3Scale.scaleUtc; // Now set the range
  ...
  var _extent = d3Array.extent(allDatums, function (datum) {
...
function buildLinearAxis(options, series, isVertical, range, outerRange) {
  var scale = options.scaleType === 'log' ? d3Scale.scaleLog() : d3Scale.scaleLinear();
  var allDatums = series.map(function (d) {
    return d.datums;
  }).flat(2);

  var _ref = options.stacked ? d3Array.extent(series.map(function (s) {
    return s.datums.map(function (datum) {
      var _datum$stackData;

      return (_datum$stackData = datum.stackData) != null ? _datum$stackData : [];
    });
  }).flat(2)) : d3Array.extent(allDatums, function (datum) {
    return options.getValue(datum.originalDatum);
  }),
      minValue = _ref[0],
      maxValue = _ref[1];
...
function buildBandAxis(options, series, isVertical, range, outerRange) {
  ...
  var scale = d3Scale.scaleBand(domain, range).round(false).paddingOuter((_options$outerBandPad2 = options.outerBandPadding) != null ? _options$outerBandPad2 : 0).paddingInner((_options$innerBandPad2 = options.innerBandPadding) != null ? _options$innerBandPad2 : 0); // Invert if necessary
...
function buildImpliedBandScale(options, scale, series, range) {
  ...
  var bandDomain = d3Array.range(bandRange / impliedBandWidth);
  var bandScale = d3Scale.scaleBand(bandDomain, range).round(false).paddingOuter((_options$outerBandPad3 = options.outerBandPadding) != null ? _options$outerBandPad3 : 0).paddingInner((_options$innerBandPad3 = options.innerBandPadding) != null ? _options$innerBandPad3 : 0);
  return bandScale;
}
...
function Cursor(props) {
  ...
  var lineSpring = web.useSpring({
    transform: translate(lineStartX, lineStartY),
    width: lineWidth + "px",
    height: lineHeight + "px",
    config: web.config.stiff // immediate: isScrolling,

  });
  var bubbleSpring = web.useSpring({
  ...
  return portalEl ? ReactDOM.createPortal(React__default.createElement("div", {
    ...
    className: "Cursor"
  }, props.options.showLine ? React__default.createElement(web.animated.div, {
  ...
  }) : null, props.options.showLabel ? React__default.createElement(web.animated.div, {
...
function TooltipRenderer(props) {
  ...
  }, d3Array.sum((_focusedDatum$group2 = focusedDatum.group) != null ? _focusedDatum$group2 : [], function (d) {
...
function Tooltip() {
  ...
  var springProps = web.useSpring(_extends({}, anchorFitStyle, {
  ...
  return show && portalEl ? ReactDOM.createPortal(React__default.createElement(web.animated.div, {
    style: springProps
  }, React__default.createElement("div", {
    ref: function ref(el) {
...
function ChartInner(_ref2) {
  ...
  var useAxisDimensionsAtom = React__default.useCallback(function () {
    ...
    return jotai.useAtom(axisDimensionsAtom);
  }, [axisDimensionsAtom]);
  var useFocusedDatumAtom = React__default.useCallback(function () {
    // eslint-disable-next-line
    return jotai.useAtom(focusedDatumAtom);
  ...
  var series = React__default.useMemo(function () {
    ...
    if (options.secondaryAxes.some(function (axisOptions) {
      ...
      options.secondaryAxes.filter(function (d) {
        ...
        var stacker = d3Shape.stack().keys(seriesIndices).value(function (_, seriesIndex, index) {
          ...
          return val;
        }).offset((_secondaryAxis$stackO = secondaryAxis.stackOffset) != null ? _secondaryAxis$stackO : d3Shape.stackOffsetNone);
  ...
  var seriesByAxisId = d3Array.sort(d3Array.groups(orderedSeries, function (d) {
